...
void OLED_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = CS | D0 | D1;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;//速度50MHz
 	GPIO_Init(GPIOA, &GPIO_InitStructure);	  //初始化GPIOD3,6
	GPIO_SetBits(GPIOA, CS | D0 | D1);

	// RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB, ENABLE);	 //使能A端口时钟
	GPIO_InitStructure.GPIO_Pin = RES | DC;
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;//速度50MHz
 	GPIO_Init(GPIOA, &GPIO_InitStructure);	  //初始化GPIOD3,6
	GPIO_SetBits(GPIOA, RES | DC);
...
void OLED_SPI_DisPlay_Off(void)
{
	...
	OLED_SPI_WR_Byte(0xAF,OLED_CMD);//关闭屏幕
}
...
void OLED_SPI_DrawLine(u8 x1,u8 y1,u8 x2,u8 y2)
{
	...
	else      //画斜线
	{
		...
		for(i=0;i<(x2-x1);i++)
			{
			  OLED_SPI_DrawPoint(x1+i,y1+i*k/10);
			}
	}
}
//x,y:圆心坐标
...
void OLED_SPI_DrawCircle(u8 x,u8 y,u8 r)
{
    ...
    while(2 * b * b >= r * r)      
    {
        OLED_SPI_DrawPoint(x + a, y - b);
        OLED_SPI_DrawPoint(x - a, y - b);
        OLED_SPI_DrawPoint(x - a, y + b);
        OLED_SPI_DrawPoint(x + a, y + b);
 
        OLED_SPI_DrawPoint(x + b, y + a);
        OLED_SPI_DrawPoint(x + b, y - a);
        OLED_SPI_DrawPoint(x - b, y - a);
        OLED_SPI_DrawPoint(x - b, y + a);
...
void OLED_SPI_ShowChar(u8 x,u8 y,u8 chr,u8 size1)
{
	...
	for(i=0;i<size2;i++)
	{
        ...
        {temp=asc2_2412[chr1][i];} //调用2412字体
				...
				for(m=0;m<8;m++)           //写入数据
				{
					if(temp&0x80)OLED_SPI_DrawPoint(x,y);
					else OLED_SPI_ClearPoint(x,y);
					temp<<=1;
					y++;
					if((y-y0)==size1)
					{
...
void OLED_SPI_ShowString(u8 x,u8 y,u8 *chr,u8 size1)
{
	while((*chr>=' ')&&(*chr<='~'))//判断是不是非法字符!
	{
		OLED_SPI_ShowChar(x,y,*chr,size1);
		x+=size1/2;
		if(x>128-size1)  //换行
		{
			x=0;
...
void OLED_SPI_ShowNum(u8 x,u8 y,u32 num,u8 len,u8 size1)
{
	...
	for(t=0;t<len;t++)
	{
		temp=(num/OLED_SPI_Pow(10,len-t-1))%10;
			if(temp==0)
			{
				OLED_SPI_ShowChar(x+(size1/2)*t,y,'0',size1);
      }
			else 
			{
			  OLED_SPI_ShowChar(x+(size1/2)*t,y,temp+'0',size1);
			}
  }
}
		...
		n++;
			for(i=0;i<size1;i++)
			{
				if(size1==16)
						...
						for(m=0;m<8;m++)
							{
								if(temp&0x01)OLED_SPI_DrawPoint(x,y);
								else OLED_SPI_ClearPoint(x,y);
								temp>>=1;
								y++;
							}
...
void OLED_SPI_ScrollDisplay(u8 num,u8 space)
{
	...
	while(1)
	{
		if(m==0)
		{
	    OLED_SPI_ShowChinese(128,24,t,16); //写入一个汉字保存在OLED_GRAM[][]数组中
			t++;
		}
		if(t==num)
			{
...
{
	OLED_SPI_WR_Byte(0xb0+y,OLED_CMD);//设置行起始地址
	OLED_SPI_WR_Byte(((x&0xf0)>>4)|0x10,OLED_CMD);
	OLED_SPI_WR_Byte((x&0x0f),OLED_CMD);
}
